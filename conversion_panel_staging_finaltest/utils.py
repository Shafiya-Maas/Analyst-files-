...
def fetch_and_save_parquet(start_date=None, end_date=None):
    if start_date and end_date:
        condition = f"AND booking_date BETWEEN '{start_date}' AND '{end_date}'"
    else:
        # Default to last 3 months if no dates are provided
        current_date = pd.to_datetime("today").date()
        three_months_ago = current_date - pd.DateOffset(months=3)
        condition = f"AND booking_date BETWEEN '{three_months_ago.date()}' AND '{current_date}'"
