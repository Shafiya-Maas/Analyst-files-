...
def run():
    ...
    with filter_col8:
        ...
        if st.button("Refresh🔄", key="refresh_button"):
            st.session_state.refresh_data = True  # This will trigger load_parquet() to refresh
                            ...
                            document.addEventListener("DOMContentLoaded", function() {{
                                ...
                                table.addEventListener('click', function(e) {{
                                    ...
                                    const matchedRows = fullData.filter(row => {{
                                        ...
                                        const activityStatusMatch = isGrandTotal || (
                                            row.Activity_Status_Final && row.Activity_Status_Final.trim().toLowerCase() === masterService.toLowerCase()
                                        );
                                    
                                        const commentsMatch = isGrandTotal || (
                                            row.comments && row.comments.trim().toLowerCase() === masterService.toLowerCase()
                                        );
                                    
                                        const vehicleTypeMatch = isGrandTotal || (
                                            row.vehicle_type && row.vehicle_type.trim().toLowerCase() === masterService.toLowerCase()
                                        );
                                    
                                        // Match if any field matches the clicked value
                                        const isMatch = serviceMatch || crmMatch || userSourceMatch || activityStatusMatch || commentsMatch || vehicleTypeMatch;
                                        ...
                                        let content = `
                                            <div style="max-height:400px; overflow-y:auto;">
                                                <table style="width:100%; border-collapse: collapse; border: 1px solid #dee2e6;">
                                                    <thead>
                                                        <tr>
                                                            <th style="padding: 8px; background-color: #f8f9fa; border: 1px solid #dee2e6;">Matched Rows</th>
